...
class SequenceProcessor:
     def __init__(self,
        kmerSize: int,sequence: str,
        blosumVersion: int = 62,
        logarithmBase: float = 2,) -> None:
     ...
     def getRepeats(self) -> list | None: 
          ...
          for i in range(0,self.kmerSize):
               kmers = []
               for ven in range(i,len(self.sequence), self.kmerSize):
                    seqKmer = self.sequence[ven:ven+self.kmerSize]
                    if len(seqKmer) == self.kmerSize :
                         kmers.append(seqKmer)
